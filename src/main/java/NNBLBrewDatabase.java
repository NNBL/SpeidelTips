...
public class NNBLBrewDatabase {
  ...
  public String findNearestEfficiency(float grainweight, float efficiency) {
    ...
    for (NNBLBrew brew : db) {
      if (brew.getGrainweight() >= (grainweight - 50) && brew.getGrainweight() <= (grainweight + 50)
        && brew.getEfficiency() >= (efficiency - 0.4) && brew.getEfficiency() <= (efficiency
        + 0.4)) {
  ...
  public String findNearestMashTemp(float temperature, float attenuation) {
    ...
    for (NNBLBrew brew : db) {
      if (brew.getMashTemp() >= (temperature - 0.5) && brew.getMashTemp() <= (temperature + 0.5)
        && brew.getAttenuation() >= (attenuation - 0.4) && brew.getAttenuation() <= (attenuation
        + 0.4)) {
    ...
    try {
      ...
      if (f.exists()) {
        BufferedReader reader =
          new BufferedReader(new InputStreamReader(new FileInputStream(pathname), "UTF8"));
        String line = null;
